...
// Query to get all sales target information
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<tr>";
                        echo "<td>" . $row['SalesTargetID'] . "</td>";
                        echo "<td>" . $row['NameofSalesTarget'] . "</td>";
                        echo "<td>" . $startDate . "</td>";
                        echo "<td>" . $endDate . "</td>";
                        echo "<td>" . $row['Description'] . "</td>";
                        echo "<td>
                                <button onclick=\"window.location.href='salestargetupdate.php?id=" . $row['SalesTargetID'] . "'\">Edit</button>
                                <button onclick=\"if(confirm('Are you sure you want to delete this sales target?')) window.location.href='salestargetdelete.php?id=" . $row['SalesTargetID'] . "';\">Delete</button>
                              </td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='6'>No sales targets found</td></tr>";
                }
                ?>
            </tbody>
        </table>

        <!-- New Button to Add Sales Target -->
        <button onclick="window.location.href='salestargetadd.php'" style="padding: 10px 20px; background-color: white; color: black; border: none; border-radius: 5px; cursor: pointer; transition: background-color 0.3s ease, transform 0.2s ease;">
